...
#### J5扩展口信号输入引脚
- 推荐使用 J5 扩展口的 **IO_L18N / G17** 作为信号输入（已在约束中示例）。如硬件接线不同，请在 `lab7.xdc` 中修改 `signal_in`。
- 下表为 J5 (2x18) 全部引脚，按实际焊接选择相应引脚并同步更新约束。
...
#### DAC0832引脚（按用户手册）
- 数据线 DI0~DI7 ：T8, R8, T6, R7, U6, U7, V9, U9
- 控制信号（低有效）：WR1=V6，WR2=R6，CS=N6
- 若您的硬件走线不同，请按实际连接同步修改 `lab7.xdc` 中 DAC 引脚
